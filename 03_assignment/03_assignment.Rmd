...
```
The first two tests present evidence for the presence of spatial error dependence and spatial lag dependence. However, once we run the 'locally robust' versions of the test, we find that (allowing for spatial error dependence) there is actually no evidence for a spatial lag dependence. Thus according to the classical specification search procedure, we should estimate a \textbf{SEM model}. 
...
The SLX model can be estimated using OLS, for example by using the plm() function from before where we include the spatially lagged values of the explanatory variables:
```{r}
...
We will now use the bsreg package to estimate the distance decay parameter. We follow the procedure layed out in @Kuschnig2022 and the accompanying GitHub repository. Using the function bslx from bsreg we estimate $\gamma$ from  @HalleckVega2015 in the code below. Since we use exactly the same data and model as in the paper corresponding to the bsreg (Kuschnig 2022), we reuse the specification for the prior parameter. 

The estimate of gamma (delta_SLX) using Bayesian methods is similar to that of @HalleckVega2015. 

```{r, echo = FALSE, results = FALSE}
library("dplyr")
library("readxl")
# Prepare the data
contig <- read_excel(file.path("data","cigarettes","Spat-Sym-US.xls"),
  col_names = paste0(1:46)) %>% as.matrix()
df <- read_excel(file.path("data","cigarettes","cigarette+2var.xls")) %>%
...
# Estimate SLX model with uncertainty around delta. 
...
saveRDS(out_slxdx, "./models/out_slxdx.rds")
```
We use an inverse gamma prior with shape and scale parameter 2. The support is (0, $\infty$). We then plot the posterior density of the connectivity parameter. We find that the posterior mean is close to 3, which is in line with @HalleckVega2015. The figure nicely visualizes the uncertainty around the connectivity parameter. The highest posterior density is from [2.7, 3.4].     

```{r, echo = FALSE, echo = FALSE, fig.align ='center'}
out_slxdx <- readRDS("./models/out_slxdx.rds")

kable(mean(out_slxdx$draws[,"delta_SLX"]), col.names = "Posterior Mean")

plot(density(out_slxdx$draws[,"delta_SLX"]) ,main = "Posterior Density")
# Calculate quantiles; for example, 25th, 50th, and 75th percentiles
quantiles <- quantile(out_slxdx$draws[,"delta_SLX"], probs = c(0.05, 0.5, 0.95))

# Add vertical lines for quantiles
abline(v = quantiles, col = c("red", "blue", "red"), lwd = 2)

# Adding legends manually, just for clarity (optional)
legend("topright", legend = paste0(c("5%", "50% Median", "95%")), 
       col = c("red", "blue", "red"), lwd = 2)

kable(rbind(quantile(out_slxdx$draws[,"delta_SLX"], p = c(0.05, 0.5, 0.95))),
      caption = "Quantile for Posterior Gamma")
```
...
The binary contiguity matrix is \textit{not} row normalized. This implies that the coefficients for the spatially lagged dependent and independent variables need to be interpreted as the direct impact of marginal changes in only \textit{one} of the neighboring cells. To obtain summary statistics of hte average partial effect and direct and indirect impacts, the spatial weight matrix has to to be considered (through row and columnwise summation).
...
fml2 = ANY_EVENT_ACLED ~ SPEI4pg + L1_SPEI4pg + L2_SPEI4pg + GSmain_ext_SPEI4pg + L1_GSmain_ext_SPEI4pg + L2_GSmain_ext_SPEI4pg + W_L2_GSmain_ext_SPEI4pg + W_L1_GSmain_ext_SPEI4pg + W_GSmain_ext_SPEI4pg + W_SPEI4pg + W_L1_SPEI4pg + W_L2_SPEI4pg + elevation_cell + 
  ...
  dis_river_cell + as.factor(shared) +  as.factor(border) + as.factor(any_mineral)+ ELF + W_elevation_cell + W_rough_cell + W_area_cell + W_ELF + as.factor(W_any_mineral) + as.factor(W_border) + as.factor(W_shared)  + 
...
mod3 <- spml(ANY_EVENT_ACLED ~ lag(ANY_EVENT_ACLED) + SPEI4pg + L1_SPEI4pg + L2_SPEI4pg + GSmain_ext_SPEI4pg + L1_GSmain_ext_SPEI4pg + L2_GSmain_ext_SPEI4pg + W_GSmain_ext_SPEI4pg +
                               W_L1_GSmain_ext_SPEI4pg + W_L2_GSmain_ext_SPEI4pg + W_SPEI4pg + W_L1_SPEI4pg + W_L2_SPEI4pg + elevation_cell + as.factor(W_border) +
...
stargazer(rbind(ar_coef_table_3, coef_table_3), header = FALSE, align = TRUE, digits = 4, title = " Replication Model 3")
stargazer(rbind(ar_coef_table_4, coef_table_4), header = FALSE, align = TRUE, out = "html", digits = 4, title = "Replication Model 4")
...
find_neighbors_horizontal <- function(centroids, max_distance = 180000, lat_tolerance = 0.01) {
...
find_neighbors_vertical <- function(centroids, max_distance = 180000, lon_tolerance = 0.01) {
  # Create a matrix to store distances
  distances <- st_distance(centroids)
...
```{r, include = FALSE, eval = FALSE}
modvertical3 <- spml(ANY_EVENT_ACLED ~ lag(ANY_EVENT_ACLED) + SPEI4pg + L1_SPEI4pg + L2_SPEI4pg + GSmain_ext_SPEI4pg + L1_GSmain_ext_SPEI4pg + L2_GSmain_ext_SPEI4pg + W_GSmain_ext_SPEI4pg + W_L1_GSmain_ext_SPEI4pg + W_L2_GSmain_ext_SPEI4pg + W_SPEI4pg + W_L1_SPEI4pg + W_L2_SPEI4pg + elevation_cell + rough_cell + area_cell + as.factor(use_primary) + dis_river_cell + as.factor(shared) +  as.factor(border) + as.factor(any_mineral) + ELF + W_elevation_cell + W_rough_cell + W_area_cell + as.factor(W_border) + W_ELF + as.factor(W_any_mineral) + as.factor(W_shared)  + W_dis_river_cell + as.factor(W_use_primary) + as.factor(country_largest_share):as.numeric(year), # country specific linear time trends
                             data = as.data.frame(geoconflict_main_if), 
                             index=c("cell","year"),
...
modelvertical4 <- spml(ANY_EVENT_ACLED ~ lag(ANY_EVENT_ACLED) + SPEI4pg + L1_SPEI4pg + L2_SPEI4pg + GSmain_ext_SPEI4pg + L1_GSmain_ext_SPEI4pg + L2_GSmain_ext_SPEI4pg + W_GSmain_ext_SPEI4pg + W_L1_GSmain_ext_SPEI4pg + W_L2_GSmain_ext_SPEI4pg + W_SPEI4pg + W_L1_SPEI4pg + W_L2_SPEI4pg + elevation_cell + rough_cell + area_cell + as.factor(use_primary) + dis_river_cell + as.factor(shared) + as.factor(W_border)+ as.factor(border) + as.factor(any_mineral) +
                              ELF + W_elevation_cell + W_rough_cell + W_area_cell + W_ELF + as.factor(W_any_mineral) + as.factor(W_shared)  + 
                              W_dis_river_cell + as.factor(W_use_primary) + as.factor(country_year_fe), # country x year FE
                             data = as.data.frame(geoconflict_main_fe), 
                             index= c("cell","year"),
...
```
We reproduced both specification 3, and 4 for both contiguity matrices. Now that we have produced a lot of tables, what do we make of that? 

The bottom line is that the results are pretty robust to different specifications of the spatial weights matrix. The main coefficient of interest (the lag of SEPI Growing Season) is statistically significant and of similar magnitude for all regressions. This is reassuring for the validity and robustness of the results. 
The coefficient for the (spatial) autoregressive parameter $\lambda$ varies somewhat depending on the specification and spatial weights matrix being used. For Model 4 $\lambda$ amounts to 0.0331, while for model 3 with the vertical spatial weights matrix $\lambda$ amounts to about 0.08. Due to the amplification of those effects this implies rather large differences in the amplification of conflict events. Nonetheless overall the results are all remarkably similar.    

## Summarize and visualize the weights matrixes and briefly explain what they imply
In both the horizontal and the vertical contiguity matrix the average degree is about 1.86 and 2.09. Thus most cells have two neighbors but some cells, when the gird is 'spherically disturbed', can have more than two neighbors.  This is because the condition that any cells is being assigned neighborhood status when the centroid is less than 180 kilometers away can include more than two cells in one direction. 

The networks implied by the vertical and horizontal contiguity matrices are sparser and less dense than the adjacency matrix used by the authors. This is also reflected in lower 'graph density' for the horizontal and vertical contiguity matrix as compared to the baseline adjacency matrix. Visualizing the spatial weight matrices further corroborates the sparser structure. 
...
In a similar spirit to @Harari2018 we simulate the effect of a \textit{one-time} and \textit{one-standard deviation SEPI growing season shock} setting all other covariates to zero (and holding everything else constant). We multiply the shock times -1 to obtain the negative impact and thus the \textit{increase} in conflict incidence. We exogenously shock a cell and calculate 'manually', based on the coefficients of model 4 (instead of model 3 as in the paper), the magnitude of the shock propagation for t = 0, 1, and 2 for all neighbors and 'neighbors neighbors'. We take into account both the dynamic, and spatial autoregressive structure of the model. We see that the shock reaches its peak effect size at t = 1, consistent with the main insight of the paper (the lagged effect of growing season shocks on conflict). A negative one standard deviation growing season shock increases conflict incidence by about 2.1 percentage points for the affected cell. Overall the spillover effects both in absolute magnitude and visually as reflected in the figure are rather small. The maximum conflict increase in a neighboring cell only amounts to about 0.5 percentage points. 
...
risk_cell_0_0 <- -1*mod4$coefficients["GSmain_ext_SPEI4pg"]*sd(geoconflict_main_weights$GSmain_ext_SPEI4pg) / (1 - sum(listw2mat(listwmat))*mod4$arcoef)
...
# we do not use those effects

risk_cell_3_0 <- risk_cell_2_0*mod4$coefficients["lag(ANY_EVENT_ACLED)"] + risk_cell_1_0*mod4$coefficients["lag(ANY_EVENT_ACLED)"]*mod4$coefficients["lag(ANY_EVENT_ACLED)"]  +   risk_cell_0_0*mod4$coefficients["lag(ANY_EVENT_ACLED)"] *mod4$coefficients["lag(ANY_EVENT_ACLED)"]*mod4$coefficients["lag(ANY_EVENT_ACLED)"]

risk_cell_3_1 <- risk_cell_1_1*mod4$coefficients["lag(ANY_EVENT_ACLED)"] + risk_cell_0_1 * mod4$coefficients["lag(ANY_EVENT_ACLED)"] *mod4$coefficients["lag(ANY_EVENT_ACLED)"] + mod4$coefficients["lag(ANY_EVENT_ACLED)"] *mod4$coefficients["lag(ANY_EVENT_ACLED)"] *risk_cell_2_1 + risk_cell_3_0*mod4$arcoef  

risk_cell_3_2 <- mod4$arcoef*risk_cell_3_1 + risk_cell_2_2*mod4$coefficients["lag(ANY_EVENT_ACLED)"] + risk_cell_1_2 * mod4$coefficients["lag(ANY_EVENT_ACLED)"] *mod4$coefficients["lag(ANY_EVENT_ACLED)"] + risk_cell_0_2*mod4$coefficients["lag(ANY_EVENT_ACLED)"] *mod4$coefficients["lag(ANY_EVENT_ACLED)"]*mod4$coefficients["lag(ANY_EVENT_ACLED)"] + mod4$arcoef*mod4$arcoef*risk_cell_3_1 + risk_cell_3_0*mod4$arcoef*mod4$arcoef
```
...
## Appendix
Functions to create horizontal and vertical contiguity neighboors list
```{r}
### Lets first create teh bianry contiguity matrix based on horizontal contiguity 
# Extract centroids of the polygons if not already point data
