...
For this we first create list, where all regions are neighbors by setting distance threshold to big enough value and then compute distances using nbdists. They are then transformed as suggested above and scaled by maximum eigenvalue as in the paper.
...
Using gamma similar to the paper, we get coefficient estimates which are very close to the paper.
