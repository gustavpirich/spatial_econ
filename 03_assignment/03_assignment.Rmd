...
The binary contiguity matrix is \textit{not} row normalized. This implies that the coefficients for the spatially lagged dependent and independent variables need to be interpreted as marginal changes in only \textit{one} of the neighboring cells. 
...
stargazer(rbind(ar_coef_table_3, coef_table_3), type = "text", align = TRUE, digits = 4, title = " Replication Model 3")

stargazer(rbind(ar_coef_table_4, coef_table_4), type = "text", align = TRUE, digits = 4, title = "Replication Model 4")
...
In both the horizontal and the vertical contiguity matrix the average degree is about 1.86 and 1.9. Thus most cells have (as expected) two neighboors but some cells , at the edge, have only one neighboors. This implies that the networks are sparser and less dense than the adjacency matrix used by the authors. This is also reflected in lower 'graph density' for the horizontal and vertical contiguity matrix as compared to the baseline adjacency matrix. Visualizing the spatial weight matrices further corroborates the sparser strucutre. 
...
Still due to the nature of dynamic multipliers in the form of the spaital autoregressive terms, even if the network appears relatively sparse and two cells are not directly linked the cells are still connected. However, the restriction of horizontal and verticalcontiguity signifcantly restricts the dynamic propragation of the effects in this context.
