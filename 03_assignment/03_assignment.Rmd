...
pacman::p_load(stringi, stringr, stringdist, haven, sf, dplyr, fuzzyjoin, 
               ...
               igraph, generics, knitr, kableExtra, formatR,readxl)
```
...
```
 ...
 Estimate the demand model, and test for spatial dependence using the procedures discussed in class, and the provided weights matrix. Suppress any theoretical considerations
— which model would the classical specification search prefer?

```{r}
data.OLS <- select(cigarette_2var, c(logc, logp,logy, starts_with("year_")|starts_with("state_")))
summary(lm(logc ~ ., data.OLS))
```
