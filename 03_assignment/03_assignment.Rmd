...
data <- c(min(area), mean(area), median(area), max(area))

kable(data)
```
The unit of observation are cells. The cells are based on a raster. At the equator the raster has a side length of 110 kilometers.  
