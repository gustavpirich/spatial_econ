...
```
We compare the matrices based on a set of characteristics. All three graphs have 103 vertices, which means they represent the same number of spatial units or locations. This allows for a direct comparison of other attributes since the baseline (number of vertices) is constant across the graphs. Distance Threshold Graph has 514 edges, indicating a higher level of connections between vertices compared to the other two. This suggests that many vertices are within the distance threshold to be considered neighbors.
...
plot(EU27$geometry, border = "darkgrey")
plot(decay_weights_matrix_list, coords, add=TRUE, col= "red", pch = 19, cex = 0.6)
