...
plot(st_geometry(EU27), border = "lightgray")
plot.nb(nblags[[1]], st_geometry(EU27), add = TRUE)
...
plot(EU27$geometry)
plot(distw, coords, add=TRUE, col="blue", cex=0.1)
...
decay_function <- function(d) exp(-1*d)
...
contig_w_matrix <- nb2mat(queen_weights, style="B", zero.policy=TRUE)
```
...
```
## Estiamte a linear regression model using OLS. 
```{r}
